package com.youtube.musica.dialog;
...
public class AddCtg extends AlertDialog {
    ...
    public AddCtg(Context context, CategoryCollection categoryCollection, CtgListener listener, CtgViewModel ctgViewModel) {
        super(context);
    ...
    private void Iniciar() {
        try {
            ...
            if(categoryCollection != null){
                ...
                editName.setText(categoryCollection.getName());
                editDetails.setText(categoryCollection.getDescription());
            }
                ...
                public void onClick(View v) {
                    if(!editName.getText().toString().trim().equals("")){ // si no esta vacio
                        if(categoryCollection == null){
                            ...
                            if(!ctgViewModel.isNameExistsExcludingCode(editName.getText().toString().trim(), categoryCollection.getName())){ // si ya existe un registro con el mismo numero
                                UpdateSata();
                            }else{
                                msg.showMsg(context.getString(R.string.exits_ctg,  editName.getText().toString()),"#FF9800",1);
                            }
    ...
    private void UpdateSata(){
        ...
        object.put("name", editName.getText().toString().trim());
        object.put("description", editDetails.getText().toString().trim());
        db.updateCtg(categoryCollection.getCode(), object).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
